...
declare function openSearchPanel(tableName: any, searchbox: string, user_id: any, access_token: any): any;
...
@Component({
  selector: 'app-news-layout1',
  templateUrl: './news-layout1.component.html',
  styleUrls: ['./news-layout1.component.css']
})
export class NewsLayout1Component {
  ...
  //   openSearchPanel(tableName, searchbox, this.user_id, this.access_token);
  ...
  onSearchNews(searchForm: NgForm) {
    ...
    if (wireValue || categoryValue || headlineValue || storytextValue || companySymbolValue) {
      ...
      wireArray.forEach((wire: any) => {
        RequestSearchNews(`@${wire}@`, categoryValue, this.searchTable, true, this.userId);
      });
  ...
  onHeadlineChange(selectedValue: string) {
    ...
    requestMenuDetails(this.user_id, this.access_token, this.mainwire);
    if (this.mainwire) {
      this.jsonData = localStorage.getItem('menu_items');
    }
    // if (menuFileName) {
